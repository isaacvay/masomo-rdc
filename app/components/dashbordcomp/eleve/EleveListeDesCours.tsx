...
interface DevoirQProps {
  ...
  schoolId: string | null;  // On reçoit le schoolId pour l'écriture
}
...
export default function DevoirQ({ devoir, onBack, initialReponses, schoolId }: DevoirQProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Enregistrement des réponses dans Firestore
      ...
      console.log('Réponses soumises:', reponses);
    ...
    onBack(); // Retour à la liste des devoirs
  };

  if (!devoir?.questions?.length) {
    return (
      <div className="p-4 text-red-500">
        Erreur: Aucune question disponible pour ce devoir.
        <button onClick={onBack} className="mt-4 text-blue-600">Retour</button>
      </div>
    );
  }
      ...
      <div className="flex items-start">
        ...
        <div className="ml-3">
          ...
          <div className="mt-2 text-sm text-yellow-700">
            <p>
              La date de remise est passée. Vous pouvez tout de même soumettre votre travail, mais il sera marqué comme en retard.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl">
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        {/* Alerte pour la soumission en retard */}
            ...
            <div className="mb-6 p-5 bg-white border border-gray-200 rounded-xl shadow-sm">
              ...
              {currentQuestion.type === 'texte' ? (
                <textarea
                  ...
                  onChange={(e) => handleReponseChange(currentQuestion.id, e.target.value)}
                  ...
                  {currentQuestion.options?.map((option) => (
                    ...
                    >
                      <input
                        ...
                        value={option.id}
                        checked={reponses[currentQuestion.id] === option.id.toString()}
                        onChange={() => handleReponseChange(currentQuestion.id, option.id.toString())}
