...
import { CheckCircle, Calendar, Clock, ArrowLeft, Save, AlertTriangle, X } from "lucide-react";
import { firestore, auth } from "@/config/firebase";
import { doc, setDoc } from "firebase/firestore";

// Types complets et extensibles
interface Subject {
  name: string;
  icon: string;
  maxima: number[];
...
interface ExamSchedule {
  ...
  examiner?: string; // Surveillant optionnel
}
...
const ExamScheduleManager = ({ selectedClass, onRetour }: HoraireExamProps) => {
  // Récupérer uniquement les matières correspondant à la classe sélectionnée
  ...
  // Récupérer l'identifiant de l'école depuis l'utilisateur authentifié
  useEffect(() => {
    const user = auth.currentUser;
    if (user) {
      // Vous pouvez adapter cette logique si votre document utilisateur possède un champ "schoolId"
      setSchoolId(user.uid);
    }
  }, []);

  // Initialiser examSchedules et charger les données sauvegardées
  useEffect(() => {
  ...
  // Validation et soumission avec sauvegarde dans Firestore en tant que sous-collection de schools
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sauvegarder dans Firestore comme sous-collection de schools
  ...
  // Générer des horaires aléatoires pour tester
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-200">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange(subject.name, activeTab, "examiner", e.target.value)
                        }
                        className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
